...
def crop_person_from_picture(pred_scores, image_index, person_index, threshold, bbox, image):
    # use just the images filtered by the threshold
    if pred_scores[image_index][person_index] > threshold:
        ...
        if width <= 0 or height <= 0:
            ...
            # round coordinates for cropping
            x1 = int(x1)
            x2 = int(x2)
            y1 = int(y1)
            y2 = int(y2)
